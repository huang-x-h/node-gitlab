...
class Jobs {
  ...
  query(projectId, jobId) {
    ...
    return this.connection.get(`/projects/${encodeURIComponent(projectId)}/jobs/${jobId}`);
  }
  ...
  trace(projectId, jobId) {
    assert(projectId, 'projectId is required.');
    assert(jobId, 'jobId is required.');
    return this.connection(`/projects/${encodeURIComponent(projectId)}/jobs/${jobId}/trace`, { json: false });
  }
   ...
   * @param {integer|string} projectId The ID or URL-encoded path of the project owned by the authenticated user
   * @param {integer} jobId The ID of a job
   * @param {string} artifactPath Path to a file inside the artifacts archive
   */
  downloadArtifactFile(projectId, jobId, artifactPath) {
    assert(projectId, 'projectId is required.');
    assert(jobId, 'jobId is required.');
    assert(artifactPath, 'artifactPath is required.');
    return this.connection.get(`/projects/${encodeURIComponent(projectId)}/jobs/${jobId}/artifacts/${artifactPath}`);
  }
}
