...
const Pipelines = require('./resources/pipelines');
...
class Gitlab {
  constructor(url, token, oauth) {
    ...
    this.connection = function(relativeUrl, options) {
      ...
      options.headers = oauth ? { Authorization: `Bearer ${token}` } : { 'Private-Token': token }
   ...
   */
  get pipelines() {
    return new Pipelines(this.connection);
  }
   ...
   */
  get jobs() {
    return new Jobs(this.connection);
  }
   ...
   */
  get repositories() {
    return new Repositories(this.connection);
  }
}
...
module.exports = {
   ...
   * @param {boolean} [oauth=false] when is true, token is oauth2 token
   * @return {Gitlab}
   */
  connect: function(url, token, oauth) {
    return new Gitlab(url, token, oauth);
  }
};
